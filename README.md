...
| ファイル名 | 用途 | テーブル | 説明 |
|-----------|------|----------|------|
| `data/main.db` | メインデータベース | `sensor_data`<br>`watering_history`<br>`camera_history`<br>`ai_consultation_history`<br>`notification_history`<br>`settings` | アプリケーションの主要データ<br>センサーデータ、履歴、設定を保存 |
| `data/scheduler.db` | スケジューラーデータベース | `apscheduler_jobs` | APSchedulerのジョブ管理<br>スケジュール設定と実行履歴を保存 |

### 🔄 データベース分離の理由

1. **責任分離**: アプリケーションデータとスケジューラーデータの分離
2. **パフォーマンス**: 独立したアクセスによる最適化
3. **保守性**: 障害分離とバックアップの分離
4. **ライブラリ制約**: APSchedulerの要求に従う
...
│                                                             │
│  ┌─────────────────┐  ┌─────────────────┐                │
│  │   main.db       │  │   scheduler.db  │                │
│  │  (メインデータ)   │  │  (スケジューラー) │                │
│  │                 │  │                 │                │
│  │ • sensor_data   │  │ • apscheduler_  │                │
│  │ • watering_     │  │   jobs          │                │
│  │   history       │  │                 │                │
│  │ • camera_       │  │                 │                │
│  │   history       │  │                 │                │
...
│  │                データフロー                              │ │
│  │                                                         │ │
│  │ センサーデータ → データマネージャー → main.db           │ │
│  │ 給水履歴     → データマネージャー → main.db           │ │
│  │ 設定データ   → データマネージャー → main.db           │ │
│  │ スケジュール → APScheduler      → scheduler.db       │ │
│  └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
